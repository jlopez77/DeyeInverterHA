...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    """
    Set up a single Total Inverter Power sensor with all other parameters as attributes.
    """
...
class DeyeInverterSensor(CoordinatorEntity, SensorEntity):
    """
    Single sensor representing the inverter, with total power as value
    and all other DYRealTime parameters as attributes.
    """
    ...
    def extra_state_attributes(self):
        """Include all other parameters from DYRealTime definitions."""
        data = self.coordinator.data or {}
        attrs = {}
        sections = (_DEFINITIONS.values() if isinstance(_DEFINITIONS, dict) else _DEFINITIONS)
