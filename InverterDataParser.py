...
def _load_definitions() -> dict | list:
    """
    Carga DYRealTime.txt, que puede ser un objeto JSON o una lista de secciones.
    """
    try:
...
def combine_registers(registers: list[int], signed: bool = True) -> int:
    """Combina registros de 16 bits en un entero de 16*n bits (complemento a dos)."""
    value = 0
...
def parse_raw(raw: list[int]) -> dict[int, float]:
    """
    Parsea la lista de registros crudos según la definición en DYRealTime.txt:
    - Aplica longitud variable, complemento a dos, factor de escala (ratio) y offset;
    - Para ítems con 'Temperature' en el título, resta 100.
    """
